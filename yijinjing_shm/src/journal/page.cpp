...
bool page::is_full() const {
    return last_frame_address() + reinterpret_cast<frame_header *>(last_frame_address())->length >
           address_border();
}
...
page_ptr page::load(const data::location_ptr &location, uint32_t dest_id, uint32_t page_id, bool is_writing,
                    bool lazy) {
  uint32_t page_size = 1 * MB; // hardcoded page size
...
std::string page::get_page_path(const data::location_ptr &location, uint32_t dest_id, uint32_t page_id) {
  ...
  return location->locator->layout_file(location, data::layout::JOURNAL, page_name);
}
